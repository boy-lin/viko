...
type WithConfig<TConfig, TConfigKey extends string> = Record<
  TConfigKey,
  TConfig
>;
type WithClearAction<TClearActionKey extends string> = Record<
  TClearActionKey,
  () => void
>;
...
>(
  ...
  CreateTaskStoreState<TTask, TConfig, TTasksKey, TConfigKey, TClearActionKey>
> {
  const {
    tasksKey,
    configKey,
    ...
    ({
      ...
      applyConfigToAllTasks: (config) => {
        set((state) => {
          ...
          const nextIndex = nextTasks.reduce<Record<string, number>>(
            (acc, task, idx) => {
              acc[task.id] = idx;
              return acc;
            },
