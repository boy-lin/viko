...
import { isVideoConfig, isAudioConfig, isImageConfig } from "@/types/converter";
...
export const ConverterItem: React.FC<ConverterItemProps> = ({ task }) => {
  ...
  const handleStart = async () => {
    ...
    if (isAudioTarget) {
      try {
        ...
        // Setup listeners
        unlistenProgressRef.current = await listen<string>(
        ...
        unlistenCompleteRef.current = await listen<string>(
          ...
          (event) => {
            ...
            const { tasks, setActiveTab, incrementUnreadFinishedCount } =
              useConverterStore.getState();
            const convertingTasks = tasks.filter(
              (t) => t.id !== task.id && t.status === "converting"
            );
        ...
        console.error(
          "Failed to start audio conversion: " + JSON.stringify(error)
        );
        updateTaskById(task.id, { status: "error" });
      }
    } else {
      ...
      try {
        ...
        unlistenCompleteRef.current = await listen<string>(
          ...
          (event) => {
            if (event.payload.includes(task.config?.outputTitle || "")) {
              ...
              const { tasks, incrementUnreadFinishedCount, setActiveTab } =
                useConverterStore.getState();
              const convertingTasks = tasks.filter(
                (t) => t.id !== task.id && t.status === "converting"
              );
        ...
        const args: any = {
          ...
          video_bitrate:
            task.config?.video?.bitrate && task.config.video.bitrate !== "auto"
        ...
        console.error(
          "Failed to start video conversion: " + JSON.stringify(error)
        );
        updateTaskById(task.id, { status: "error" });
      }
    }
  ...
  const handleFormatChange = (updates: FormatSelectorValue) => {
    const { updateUnfinishedTaskConfig } = useConverterStore.getState();
    ...
    if (newFormatType === "video") {
      const existingVideo = isVideoConfig(task.config)
        ? task.config.video
        : {
            encoder: "h264",
      ...
      newConfig = {
        type: "video",
        outputFormat: updates.outputFormat || task.config.outputFormat,
        outputTitle: task.config.outputTitle,
        video: {
          ...existingVideo,
          resolution: updates.resolution || existingVideo.resolution,
          encoder: updates.videoEncoder || existingVideo.encoder,
        },
        audioTracks: isVideoConfig(task.config)
          ? task.config.audioTracks
          : undefined,
      };
    } else if (newFormatType === "audio") {
      const existingAudioTracks = isAudioConfig(task.config)
        ? task.config.audioTracks
        : task.config.audioTracks && task.config.audioTracks.length > 0
        ? task.config.audioTracks
        : [
            {
              trackIndex: 0,
      ...
      const existingImage = isImageConfig(task.config)
        ? task.config.image
        : { quality: "80" };
    ...
    updateUnfinishedTaskConfig(task.id, newConfig);
  };
  ...
  return (
    <>
      <div className="bg-secondary/20 border border-border rounded-xl p-4 flex gap-4 hover:border-purple-300 transition-colors group">
        {/* Thumbnail */}
        <MediaThumbnail path={task.path} title={task.title} isVideo={isVideo} />
