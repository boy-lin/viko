...
export const ConversionSettingsDialog: React.FC<ConversionSettingsDialogProps> = ({
  task,
  open,
  onOpenChange,
}) => {
  ...
  const updateAudioTrack = (index: number, field: string, value: string) => {
    const newTracks = [...config.audioTracks];
      ...
      <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
        <DialogHeader className="flex flex-row items-center justify-between space-y-0 pb-4 border-b">
          <div className="flex-1 flex items-center gap-2 mr-8">
            <Label htmlFor="title" className="shrink-0 text-muted-foreground">Title:</Label>
            <div className="relative flex-1">
              <Input
                id="title"
                value={config.outputTitle}
                onChange={(e) => setConfig({ ...config, outputTitle: e.target.value })}
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-x-8 gap-y-4">
                <VideoEncoderSelect
                  ...
                  onValueChange={(v) => setConfig({ ...config, video: { ...config.video, encoder: v } })}
                />

                <VideoResolutionSelect
                  value={config.video?.resolution}
                  onValueChange={(v) => setConfig({ ...config, video: { ...config.video, resolution: v } })}
                />

                <VideoFrameRateSelect
                  value={config.video?.frameRate}
                  onValueChange={(v) => setConfig({ ...config, video: { ...config.video, frameRate: v } })}
                />

                <VideoBitrateSelect
                  value={config.video?.bitrate}
                  onValueChange={(v) => setConfig({ ...config, video: { ...config.video, bitrate: v } })}
                />
              </div>
                ...
                {config.audioTracks.length > 0 ? (
                  <div className="grid grid-cols-2 gap-x-8 gap-y-4">
                    ...
                    <AudioSampleRateSelect
                      ...
                      onValueChange={(v) => updateAudioTrack(0, 'sampleRate', v)}
                    />
                ...
                <div key={index} className="space-y-4">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      ...
                      />
                      <Label htmlFor={`audio-check-${index}`} className="font-bold text-lg cursor-pointer">Audio Track {index + 1}</Label>
                    </div>
                  ...
                  <div className="grid grid-cols-2 gap-x-8 gap-y-4">
                    <AudioEncoderSelect
                      value={track.encoder}
                      onValueChange={(v) => updateAudioTrack(index, 'encoder', v)}
                      format={outputFormat}
                    />

                    <AudioChannelSelect
                      value={track.channels}
                      onValueChange={(v) => updateAudioTrack(index, 'channels', v)}
                    />

                    <AudioSampleRateSelect
                      value={track.sampleRate}
                      onValueChange={(v) => updateAudioTrack(index, 'sampleRate', v)}
                    />

                    <AudioBitrateSelect
                      value={track.bitrate}
                      onValueChange={(v) => updateAudioTrack(index, 'bitrate', v)}
                    />
                  </div>
                </div>
              ))
